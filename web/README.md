# `doacao` — the seed for AngularJS apps
...
To get you started you can simply clone the `doacao` repository and install the dependencies:

### Prerequisites

You need git to clone the `doacao` repository. You can get git from [here][git].

We also use a number of Node.js tools to initialize and test `doacao`. You must have Node.js
and its package manager (npm) installed. You can get them from [here][node].

### Clone `doacao`

Clone the `doacao` repository using git:

```
git clone https://github.com/angular/doacao.git
cd doacao
```

If you just want to start a new project without the `doacao` commit history then you can do:

```
git clone --depth=1 https://github.com/angular/doacao.git <your-project-name>
```
...
There are two kinds of tests in the `doacao` application: Unit tests and end-to-end tests.

### Running Unit Tests

The `doacao` app comes preconfigured with unit tests. These are written in [Jasmine][jasmine],
...
The `doacao` app comes with end-to-end tests, again written in [Jasmine][jasmine]. These tests
...
up-to-date. The `doacao` project is configured to do this automatically before running the
...
The `doacao` project supports loading the framework and application scripts asynchronously.
...
The `doacao` project comes preconfigured with a local development web server. It is a Node.js
...
your repository and execute scripts such as building the app or running tests. The `doacao`
